...
VisionSubsystem::VisionSubsystem(std::function<frc::Pose3d()> accessor, std::function<void(frc::Pose3d, units::millisecond_t)> updater) :
  ...
  m_camera{"photonvision"},
  m_poseEstimator{kTagLayout, photon::MULTI_TAG_PNP_ON_COPROCESSOR, kRobotToCam},
  m_accessor{accessor},
  m_updater{updater}
{
...
void VisionSubsystem::ProcessCameraResults(std::function<void(frc::Pose3d, units::millisecond_t)> updater) {
  for (const auto& result : m_camera.GetAllUnreadResults()) {
    if (result.HasTargets()) {
      auto visionEst = m_poseEstimator.Update(result);

      if (visionEst.has_value()) {
        updater(visionEst->estimatedPose, visionEst->timestamp);
      }
    }
}
}
